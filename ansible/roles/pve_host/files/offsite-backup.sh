...
rm -rf $S; mkdir -p $S/docker-pve $S/docker-vm $S/minecraft; chmod 700 $S
ssh $O leila@192.168.0.115 'sudo tar czf - -C /opt --exclude=stacks/forgejo/data --exclude=stacks/n8n/backups --exclude=stacks/monitoring/prometheus --exclude=stacks/monitoring/grafana --warning=no-file-changed stacks; r=$?; [ $r -le 1 ]' > $S/docker-pve/stacks.tar.gz || fail "docker-pve stacks"
ssh $O leila@192.168.0.115 'sudo cat /opt/stacks/monitoring/grafana/data/grafana.db' > $S/docker-pve/grafana.db || fail "grafana.db"
ssh $O leila@192.168.0.113 'sudo tar czf - -C /opt --exclude=stacks/ilust/data --warning=no-file-changed stacks; r=$?; [ $r -le 1 ]' > $S/docker-vm/stacks.tar.gz || fail "docker-vm stacks"
